...
public:
	...
	//シャドウマップ描画関数.
	void ShadowRender(const D3DXMATRIX& mView,
		const D3DXMATRIX& mProj,
		const LIGHT& Light,
		const D3DXVECTOR3& CamPos,
		const LPD3DXANIMATIONCONTROLLER pAC);	//nullptrでデフォルト使用する.
	...
	const D3DXMATRIX& GetRotation()		{ return m_mRotation;		}	

	void SetRot(const D3DXMATRIX& Rot) { m_mRotation = Rot;  }
	...
	LPD3DXMESH GetForRay() { return m_ModelForRay.pMesh; }

	void SetQuaterRot(bool isQ) { m_IsQuater = isQ; }
...
private:
	...
	//シャドウフレーム描画.
	void DrawShadowFrame(LPD3DXFRAME pFrame);

	//パーツ描画.
	void DrawPartsMesh( SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer );
	//シャドウパーツ描画.
	void DrawShadowPartsMesh(SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer);
...
//	SKIN_PARTS_MESH* m_pReleaseMaterial;
	...
	MY_MODEL	m_ModelForRay;

	D3DXVECTOR3 m_DefaPos;

	//シャドウマップ用シェーダー
	ID3D11VertexShader*		m_pShadowVertexShader;
	ID3D11InputLayout*		m_pShadowVertexLayout;
