	...
	, m_Hp			(100.0f)
{
	...
	m_pRayY = new RAY();
	m_pRayY->Axis = D3DXVECTOR3(0.0f, -1.0f, 0.0f);
	m_pRayY->Length = 1.0f;
	...
	LIGHT& Light, CAMERA& Camera)
{
	...
	//レンダリング.
	m_pMesh->Render(View, Proj, Light, Camera.Position, m_pAnimCtrl);
}

void CSkinMeshObject::DrawShadowMap()
{
	if (m_pMesh == nullptr) {
		return;
	}

	D3DXMATRIX	view = CCamera::GetInstance()->GetViewMatrix();
	D3DXMATRIX	proj = CCamera::GetInstance()->GetProjMatrix();
	D3DXVECTOR3 campos = CCamera::GetInstance()->GetCamera().Position;
	...
	//レンダリング.
	m_pMesh->ShadowRender(view, proj, CLightCamera::GetInstance()->GetLight(), campos, m_pAnimCtrl);

}

void CSkinMeshObject::QuaterDraw(D3DXMATRIX& View, D3DXMATRIX& Proj, LIGHT& Light, CAMERA& Camera)
{
	if (m_pMesh == nullptr) {
		return;
	}

	m_pMesh->SetQuaterRot(true);
	...
	D3DXVECTOR3 extractedRotaion;

	D3DXMATRIX mRot = m_pMesh->GetRotation();

	//必要になったらやる
	//extractedRotaion.z = atan2f(mRot.m[1][0], mRot.m[0][0]);
	//extractedRotaion.x = asinf(-mRot.m[2][0]);

	extractedRotaion.y = atan2f(-mRot._31, -mRot._33);

	m_QuaterRot = extractedRotaion;
...
bool CSkinMeshObject::IsHitForRay(const RAY& pRay, float* pfDistance, D3DXVECTOR3* pvIntersect, D3DXVECTOR3* pNormal)
{
...
//壁からの位置を計算する.
void CSkinMeshObject::CalculatePositionFromWall(CROSSRAY* pCrossRay)
{
	static constexpr float WSPACE = 1.8f;	//壁との限界距離.
...
//交差位置のポリゴンの頂点を見つける.
//※頂点座標はローカル座標.
HRESULT CSkinMeshObject::FindVerticesOnPoly(
	LPD3DXMESH pMesh,
	DWORD dwPolyIndex,
...
//回転値調整(1周以上しているときの調整).
void CSkinMeshObject::ClampDirection(float* dir)
{
	if (*dir > D3DX_PI * 2.0f)
	{
		//一周以上している/
