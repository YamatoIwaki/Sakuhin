...
HRESULT CDirectX11::Create(HWND hWnd)
{
	...
	//	ビューポート設定.
	//------------------------------------------------
	m_ViewPort.Width	= WND_W;	//幅.
	m_ViewPort.Height	= WND_H;	//高さ.
	m_ViewPort.MinDepth = 0.0f;		//最小深度（手前）.
	m_ViewPort.MaxDepth = 1.0f;		//最大深度（奥）.
	m_ViewPort.TopLeftX = 0.0f;		//左上位置x.
	m_ViewPort.TopLeftY = 0.0f;		//左上位置y.

	m_pContext11->RSSetViewports( 1, &m_ViewPort );
