	...
	LIGHT& Light, D3DXVECTOR3& CamPos)
{
...
#else
	D3DXVECTOR3 vecAxisX = D3DXVECTOR3(1.f, 0.f, 0.f);
	D3DXVECTOR3 vecAxisY = D3DXVECTOR3(0.f, 1.f, 0.f);
	D3DXVECTOR3 vecAxisZ = D3DXVECTOR3(0.f, 0.f, 1.f);

	//クォータニオン回転
	D3DXQUATERNION qX, qY, qZ;
	D3DXQuaternionRotationAxis(&qX, &vecAxisX, m_Rotation.x);
	D3DXQuaternionRotationAxis(&qY, &vecAxisY, m_Rotation.y);
	D3DXQuaternionRotationAxis(&qZ, &vecAxisZ, m_Rotation.z);

	D3DXQUATERNION qRot = qY * qX * qZ;
	...
	m_pContext11->PSSetShaderResources(1, 1, CShadowMap::GetInstance()->GetShadowMapSRV());
...
void CStaticMesh::ShadowRender()
{
	...
	//使用するシェーダのセット.
	m_pContext11->VSSetShader(CShadowMap::GetInstance()->GetVertexShader(), nullptr, 0);	//頂点シェーダ.
	m_pContext11->PSSetShader(CShadowMap::GetInstance()->GetPixcelShader(), nullptr, 0);	//ピクセルシェーダ.

	//メッシュのレンダリング.
	ShadowRenderMesh(mWorld);
	...
	D3DXMATRIX& mWorld, D3DXMATRIX& mView, D3DXMATRIX& mProj)
{
		...
		0, &pData )))
	{
		...
		D3DXMATRIX mLightView = CLightCamera::GetInstance()->GetViewMatrix();
		D3DXMATRIX mLightProjection = CLightCamera::GetInstance()->GetProjMatrix();

		cb.mWLP = mWorld * mLightView * mLightProjection;
		...
		cb.mWLVPT = mWorld * mLightView * mLightProjection * mClipToUV;
		D3DXMatrixTranspose(&cb.mWLVPT, &cb.mWLVPT);
...
void CStaticMesh::ShadowRenderMesh(D3DXMATRIX& mWorld)
{
	ID3D11Buffer* pShadowCB = CShadowMap::GetConstantBuffer();
	...
	if (SUCCEEDED(m_pContext11->Map(
		...
		0, &pData)))
	{
		CShadowMap::CBUFFER_PER_MESH cb;

		//ワールド,ビュー,プロジェクション行列を渡す.
		//D3DXMATRIX mWVP = mWorld * mView * mProj;
		D3DXMATRIX mWVP = mWorld * CLightCamera::GetInstance()->GetViewMatrix() * CLightCamera::GetInstance()->GetProjMatrix();
		D3DXMatrixTranspose(&mWVP, &mWVP);	//行列を転置する.
		//※行列の計算方法がDirectXとGPUで異なるため転置が必要.
		cb.mWLVP = mWVP;
		...
		//バッファ内のデータの書き換え終了時にUnmap.
		m_pContext11->Unmap(pShadowCB, 0);
	}

	//このコンスタントバッファをどのシェーダで使用するか？.
	m_pContext11->VSSetConstantBuffers(0, 1, &pShadowCB);	//頂点シェーダ.
	m_pContext11->PSSetConstantBuffers(0, 1, &pShadowCB);	//ピクセルシェーダ.

	//頂点インプットレイアウトをセット.
	m_pContext11->IASetInputLayout(CShadowMap::GetInstance()->GetLayoutShader());
