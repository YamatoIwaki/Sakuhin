...
const TCHAR SHADOW_SHADER_NAME[] = _T("Data\\Shader\\SkinMeshShadow.hlsl");
...
CSkinMesh::~CSkinMesh()
{
	...
	SAFE_RELEASE(m_pShadowVertexShader);
	SAFE_RELEASE(m_pShadowPixelShader);
	SAFE_RELEASE(m_pShadowVertexLayout);
...
HRESULT	CSkinMesh::CreateShader()
{
	...
	if (FAILED(
		D3DX11CompileFromFile(
			SHADOW_SHADER_NAME, nullptr, nullptr,
			"VS_Main", "vs_5_0",
			uCompileFlag, 0, nullptr,
			&pCompiledShader, &pErrors, nullptr)))
	{
	...
	if (FAILED(
		m_pDevice11->CreateInputLayout(
			layout, numElements, pCompiledShader->GetBufferPointer(),
			pCompiledShader->GetBufferSize(), &m_pShadowVertexLayout)))
	{
		_ASSERT_EXPR(false, L"頂点インプットレイアウト作成失敗");
		return E_FAIL;
	}
	//頂点インプットレイアウトをセット
	m_pContext11->IASetInputLayout(m_pShadowVertexLayout);
...
HRESULT CSkinMesh::LoadXMesh( LPCTSTR FileName )
{
	...
	lstrcpy(m_ModelForRay.FileName, FileName);
...
void CSkinMesh::ShadowRender(const D3DXMATRIX& mView, const D3DXMATRIX& mProj, const LIGHT& Light, const D3DXVECTOR3& CamPos, const LPD3DXANIMATIONCONTROLLER pAC)
{
	...
	D3DXMatrixIdentity(&m);
	m_pD3dxMesh->UpdateFrameMatrices(m_pD3dxMesh->m_pFrameRoot, &m);
	DrawShadowFrame(m_pD3dxMesh->m_pFrameRoot);
...
//シャドウ用
VOID CSkinMesh::DrawShadowFrame(LPD3DXFRAME p)
{
	MYFRAME*			pFrame	= reinterpret_cast<MYFRAME*>( p );
	SKIN_PARTS_MESH*	pPartsMesh	= pFrame->pPartsMesh;
	MYMESHCONTAINER*	pContainer	= reinterpret_cast<MYMESHCONTAINER*>( pFrame->pMeshContainer );
	...
	if( pFrame->pFrameSibling != nullptr )
	{
		DrawShadowFrame( pFrame->pFrameSibling );
	}
	//(親子)
	if( pFrame->pFrameFirstChild != nullptr )
	{
		DrawShadowFrame( pFrame->pFrameFirstChild );
	}
...
//シャドウ用
void CSkinMesh::DrawShadowPartsMesh(SKIN_PARTS_MESH* pMesh, D3DXMATRIX World, MYMESHCONTAINER* pContainer)
{
	...
	////使用するシェーダのセット.
	m_pContext11->VSSetShader(m_pShadowVertexShader, nullptr, 0);
	m_pContext11->PSSetShader(m_pShadowPixelShader, nullptr, 0);
...
void CSkinMesh::CalcWorldMatrix()
{
	...
	else
	{
		...
		//クォータニオンを初期化
		D3DXQUATERNION q_current, q_target, q_interpolated;

		//方向を出す
		D3DXVECTOR3 direction = m_DefaPos - m_Position;

		//現在の方向をクォータニオンに変換.
		D3DXMatrixRotationYawPitchRoll(&m_mRotation, atan2(direction.x, direction.z), 0, 0);
		D3DXQuaternionRotationMatrix(&q_current, &m_mRotation);

		//ターゲット方向をクォータニオンに変換
		D3DXVECTOR3 normalizedTargetDir;
		D3DXVec3Normalize(&normalizedTargetDir, &m_Rotation);

		D3DXVECTOR3 rotationAxis;
		D3DXVec3Cross(&rotationAxis, &direction, &normalizedTargetDir);

		float rotationAngle = acosf(D3DXVec3Dot(&direction, &normalizedTargetDir));
		D3DXQuaternionRotationAxis(&q_target, &rotationAxis, rotationAngle);

		//現在のクォータニオンと目標クォータニオンの間で徐々に補間
		float RotSp = 1.0f;		//回転速度.
		float t = RotSp / FPS;
		D3DXQuaternionSlerp(&q_interpolated, &q_current, &q_target, t);

		D3DXMatrixRotationQuaternion(&m_mRotation, &q_interpolated);
...
void CSkinMesh::SendCBufferPerShadowMesh()
{
	D3D11_MAPPED_SUBRESOURCE pDat;
			...
			D3D11_MAP_WRITE_DISCARD, 0, &pDat)))
	{
		...
		cb.mWVP = m_mWorld * m_mView * m_mProj;
		D3DXMatrixTranspose(&cb.mWVP, &cb.mWVP);	//行列を転置する.
		cb.mWLVP = CLightCamera::GetInstance()->GetViewMatrix() * CLightCamera::GetInstance()->GetProjMatrix();
		D3DXMatrixTranspose(&cb.mWLVP, &cb.mWLVP);	//行列を転置する.
		//※行列の計算方法がDirectXとGPUで異なるため転置が必要.
...
void CSkinMesh::QuaterSetDefaPos()
{
	//方向を出す
	D3DXVECTOR3 direction = m_DefaPos - m_Position;
	...
	m_DefaPos = m_Position + vecAxisZ;	//中心地を更新
