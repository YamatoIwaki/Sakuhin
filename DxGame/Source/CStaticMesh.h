...
public:
	...
	struct CBUFFER_PER_MESH
	{
		...
		D3DXMATRIX	mWLP;		//ワールド,ライトビュー,プロジェクションの合成変換行列..
		D3DXMATRIX	mWLVPT;		//ワールド,ライトビュー,プロジェクション,UVの合成変換行列.	
	};
	//マテリアル単位で渡す情報.
...
private:
	...
	//シャドウマップ用レンダリング関数(クラス内でのみ使用する).
	void ShadowRenderMesh(D3DXMATRIX& mWorld);
...
private:
	...
	ID3D11VertexShader* m_pShadowVertexShader;	//シャドウマップ用頂点シェーダ.
	ID3D11InputLayout*	m_pShadowVertexLayout;		//シャドウマップ用頂点レイアウト.
	ID3D11PixelShader*	m_pShadowPixelShader;		//シャドウマップ用ピクセルシェーダ.
